...
struct Node * Node_FindNext(struct Node * begin_search, elem_t value_to_find)
{
    ...
    assert(begin_search);
    assert(value_to_find);
    struct Node * curr_ptr = begin_search;

#ifdef _ASM_TEST_
    asm(
    "mov rdx, rdi;"         // В rcx - указатель на текущий узел
    "mov rsi, [rcx];"       // В rsi кладем указатель на текущую строку
    "go:"
    "cmp [rsi + 8], rax;"   // Сравниваем длины, если нет, то на след. узел
    "jne to_next;"
    "mov rsi, [rsi]"
    ";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
    "strcmp:"
    ...
    ";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
    "to_next:"
    "mov rcx, [rcx + 8];"   // Переходим к следующему узлу
    "test rcx, rcx;"           // Проверяем на конец списка
    "je quit;"
    "mov rsi, [rcx];"       // В rsi кладем начало структуры Line
    "mov rdi, rdx;"         // Восстанавливаем rdi
    "jmp go;"
    "quit:"
    :"=c"(curr_ptr)
    :"D"(value_to_find->line_begin), "c"(curr_ptr), "a"(value_to_find->len)
    );
    return curr_ptr;
    asm(                   // Версия с repe scasb
            "mov rdx, rdi;"
            "mov rsi, [rbx];"
            "go:"
            "cmp [rsi + 8], rcx;"
            ...
            "quit:"
            :"=a"(curr_ptr)
            :"D"(value_to_find->line_begin), "b"(curr_ptr), "c"(value_to_find->len)
            :
            );
    return curr_ptr;
...
#else
    while (curr_ptr) {
        if (curr_ptr->data->len == value_to_find->len)
            if (strcmp(curr_ptr->data->line_begin, value_to_find->line_begin) == 0)
                return curr_ptr;
...
int ListOk(struct List * my_list)
{
    ...
    if (my_list -> size < 0)
    {
        printf("Invalid size, your_list -> size = %ld\n", my_list -> size);
        return INVALID_SIZE;
    }
